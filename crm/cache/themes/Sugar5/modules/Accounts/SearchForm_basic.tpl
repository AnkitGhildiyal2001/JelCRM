...
<input type='text' name='{$fields.name_basic.name}' 
	...
	<td scope="row" nowrap="nowrap" width='1%' >
		...
		<label for='companyname_c_basic' >{sugar_translate label='LBL_COMPANYNAME' module='Accounts'}</label>
    	</td>

	
	<td  nowrap="nowrap" width='1%'>
			
{if strlen($fields.companyname_c_basic.value) <= 0}
{assign var="value" value=$fields.companyname_c_basic.default_value }
{else}
{assign var="value" value=$fields.companyname_c_basic.value }
{/if}  
<input type='text' name='{$fields.companyname_c_basic.name}' 
    id='{$fields.companyname_c_basic.name}' size='30' 
    maxlength='255' 
    value='{$value}' title=''      >
   	   	</td>
