...
id='{$fields.taxid_c.name}' size='30' maxlength='11' value='{$value}' title='9-Digit TAX ID' tabindex='0'    >
</tr>
{/capture}
{if $fieldsUsed > 0 }
{$tableRow}
...
{/if}  
<input type='text' name='{$fields.claimnumber_c.name}' 
id='{$fields.claimnumber_c.name}' size='30' 
maxlength='255' 
value='{$value}' title=''      >
</tr>
{/capture}
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.jel_percent_c.name}" 
id="{$fields.jel_percent_c.name}" 
title=''       
>
{if isset($fields.jel_percent_c.value) && $fields.jel_percent_c.value != ''}
{html_options options=$fields.jel_percent_c.options selected=$fields.jel_percent_c.value}
{else}
{html_options options=$fields.jel_percent_c.options selected=$fields.jel_percent_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.jel_percent_c.options }
{capture name="field_val"}{$fields.jel_percent_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.jel_percent_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.jel_percent_c.name}" 
id="{$fields.jel_percent_c.name}" 
title=''          
>
{if isset($fields.jel_percent_c.value) && $fields.jel_percent_c.value != ''}
...
<input
id="{$fields.jel_percent_c.name}-input"
name="{$fields.jel_percent_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.jel_percent_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.jel_percent_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.jel_percent_c.name}-input', '{$fields.jel_percent_c.name}');sync_{$fields.jel_percent_c.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.jel_percent_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.jel_percent_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.jel_percent_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.jel_percent_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.jel_percent_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.jel_percent_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.jel_percent_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
addToValidate('EditView', 'claimnumber_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_CLAIMNUMBER' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'multitaxid_c', 'enum', false,'{/literal}{sugar_translate label='LBL_MULTITAXID' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'altsignertitle_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_ALTSIGNERTITLE' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'claimfilingnumber_c', 'enum', false,'{/literal}{sugar_translate label='LBL_CLAIMFILINGNUMBER' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'taxid2_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_TAXID2' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'claimstatus_c', 'enum', false,'{/literal}{sugar_translate label='LBL_CLAIMSTATUS' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'leads_accounts_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_LEADS_ACCOUNTS_1_FROM_ACCOUNTS_TITLE' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'cases_c', 'enum', true,'{/literal}{sugar_translate label='LBL_CASES' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'estclaim_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_ESTCLAIM' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'multilocation_c', 'enum', false,'{/literal}{sugar_translate label='LBL_MULTILOCATION' module='Leads' for_js=true}{literal}' );
...
addToValidate('EditView', 'companyname_c', 'varchar', true,'{/literal}{sugar_translate label='LBL_COMPANYNAME' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'office_phone2_c', 'phone', false,'{/literal}{sugar_translate label='LBL_OFFICE_PHONE2' module='Leads' for_js=true}{literal}' );
...
addToValidate('EditView', 'estimateform_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_ESTIMATEFORM' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'taxid_c', 'int', false,'{/literal}{sugar_translate label='LBL_TAXID' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'dbaname_c', 'varchar', true,'{/literal}{sugar_translate label='LBL_DBANAME' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'team_lead_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_TEAM_LEAD' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'phone_mobile_new_c', 'phone', false,'{/literal}{sugar_translate label='LBL_PHONE_MOBILE_NEW' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'chapter_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_CHAPTER' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'jel_percent_c', 'enum', false,'{/literal}{sugar_translate label='LBL_JEL_PERCENT' module='Leads' for_js=true}{literal}' );
addToValidate('EditView', 'team_lead_email_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_TEAM_LEAD_EMAIL' module='Leads' for_js=true}{literal}' );
