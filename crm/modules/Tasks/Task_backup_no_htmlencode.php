...
class Task extends SugarBean
{
    ...
    public function generateClientAgreement()
    {
        ...
        if ($lead_id != "") {
            ...
            $document->setValue('dateDay', self::decodeString($day));
            $document->setValue('dateMonth', self::decodeString($month));
            $document->setValue('dateYear', self::decodeString($year));
            $document->setValue('companyLegalName', self::decodeString($lead_legal_name));
            $document->setValue('companyAddress', self::decodeString($lead_address));
            $document->setValue('accountExecutive', self::decodeString($ae));
            $document->saveAs($userAgreementFile);

            $estimate = new \PhpOffice\PhpWord\TemplateProcessor($userEstimateFile);
            $estimate->setValue('companyLegalName', self::decodeString($lead_legal_name));
            $estimate->setValue('companyLegalName', self::decodeString($lead_legal_name));
            $estimate->setValue('companyAddress', self::decodeString($lead_address));
            $estimate->setValue('companyAddress', self::decodeString($lead_address));
            $estimate->setValue('contactName', self::decodeString($contact_name));
            $estimate->setValue('contactName', self::decodeString($contact_name));
            $estimate->setValue('contactEmail', self::decodeString($contact_email));
            $estimate->setValue('contactEmail', self::decodeString($contact_email));
            $estimate->setValue('contactPhone', self::decodeString($contact_phone));
            $estimate->setValue('contactPhone', self::decodeString($contact_phone));
    ...
    /**
     * Sourced: https://runningwebgeek.com/2015/12/phpword-round-2/
     */
    static public function decodeString($str){
        $chr_map = array(
