...
class Task extends SugarBean
{
    ...
    public $descriptionFile = 'modules/Tasks/DocGeneration/Templates/MCVisa_description';
    public $estimateFile = 'modules/Tasks/DocGeneration/Templates/MCVisa_estimate_form';
            ...
            if ($lead->cases_c == 'VisaMC2018') {
                if ($agreement = $this->generateClientAgreement()) {
                    if ($agreement !== 'AR') {
                        ...
                        $vp = (new User())->retrieve($_POST['assigned_user_id']);

                        $team_lead = $ae->teamleader_c;
                        $team_lead = explode(" ", $team_lead);

                        $query = "SELECT * FROM users WHERE first_name = '{$team_lead[0]}' AND last_name = '{$team_lead[1]}'";
                        $result = $this->db->query($query);
                        $team_lead = $this->db->fetchByAssoc($result);
                        if (!empty($team_lead)) {
                            $team_lead = (new User())->retrieve($team_lead['id']);
                            $team_lead = $team_lead->email1;
                        } else {
                            $team_lead = '';
                        }
                        ...
                        $mail->FromName = 'JEL - TEST MESSAGE';
                        $mail->Subject = 'JEL Client Agreement';
                        $mail->Body = '[THIS IS A TEST MESSAGE -- PLEASE IGNORE] Attached is your client agreement. Please sign and return.';
                        ...
                        $mail->AddAddress($lead->email1, 'Client'); //client email
                        $mail->AddCC('jermaine@jkhannans.com', 'Developer'); //developer -- will be removed
                        $mail->AddCC('jermainehannans@outlook.com', 'Developer'); //developer -- will be removed
                        $mail->AddCC($ae->email1, 'AE1'); //AE email
                        $mail->AddCC($ae->email2, 'AE2'); //AE email
                        $mail->AddCC($team_lead, 'Team Leader'); //Team leader email
                        $mail->AddCC($vp->email1, 'VP'); //VP email
                        $mail->Send();

                        $_REQUEST['name'] .= ' (AUTO-SENT)';
                        $_REQUEST['description'] .= "\n\n [THIS AGREEMENT WAS SCHEDULED TO BE AUTO-SENT. PLEASE VERIFY WITH AE AND TEAM LEADER.]";

                    } else {
                        $_REQUEST['name'] .= ' (ADMIN RQRD)';
                        $_REQUEST['description'] .= "\n\n [SEND INFOPACK MUST BE SUBMITTED BY AN ADMIN -- VERIFY PERCENTAGE IN LEAD DETAILS.]";
                    }

                }
            } else {
                $_REQUEST['name'] .= ' (MANUAL SEND RQRD)';
                $_REQUEST['description'] .= "\n\n [THIS AGREEMENT MUST BE SENT MANUALLY AS THIS AGREEMENT TYPE HAS NOT BEEN AUTOMATED.]";
            }
        }
    ...
    public function setLeadValues()
    {
        ...
        if ($_REQUEST['target_action'] == "QuickCreate") {
            ...
            $lead_address = "$lead_street, $lead_city, $lead_zip, $lead_state";
    ...
    public function generateClientAgreement()
    {
        ...
        if ($lead_id != "") {
            ...
            //legal name
            $lead_legal_name = $lead->companyname_c;
